...
export default function ExhibitionModal({ isOpen, onClose, exhibition }: Props) {
	...
	const mapLink = useMemo(() => {
		...
		if (position)
			return generateMapsLink(
				exhibition.location[0],
	...
	return (
		...
		>
			...
			<svg
				className={`${styles.blobBlue} ${styles.blob}`}
				width="231"
				height="302"
				viewBox="0 0 231 302"
				fill="none"
			...
			>
				<path
					d="M137.032 -10.1637C157.006 -33.6689 193.678 -37.9182 220.938 -23.7662C248.357 -9.77402 266.524 22.4595 276.357 52.7537C286.191 83.0479 287.53 111.242 291.274 140.242C295.018 169.243 301.006 199.208 286.46 213.755C272.074 228.141 236.993 227.268 210.586 238.586C184.5 249.905 166.77 273.736 138.929 288.415C111.088 303.414 72.9773 309.102 65.5648 287.623C57.9924 266.303 81.2783 217.657 66.6289 182.227C52.1394 146.637 -0.285391 124.264 0.972278 112.734C1.90958 101.203 56.5294 100.515 85.1564 78.6289C114.104 56.744 117.218 13.502 137.032 -10.1637Z"
					fill="#D0EBFC"
					fillOpacity="0.4"
				/>
			</svg>
			...
			>
				<path
					d="M140.81 43.6742C163.699 63.8436 190.148 75.7079 188.961 91.9791C187.605 108.081 158.443 128.42 148.44 153.674C138.267 178.928 147.253 209.097 138.776 222.826C130.468 236.555 104.527 233.843 78.9258 233.674C53.3243 233.335 27.8924 235.538 10.4291 224.69C-6.86463 213.843 -16.1897 190.114 -18.3938 167.742C-20.7674 145.199 -16.1897 124.182 -4.32144 110.962C7.54681 97.7418 26.7056 92.4875 41.7952 69.7758C56.8848 46.8945 67.7358 6.72514 82.995 0.792953C98.2541 -5.13923 117.752 23.5047 140.81 43.6742Z"
					fill="#C3E1CC"
					fillOpacity="0.4"
				/>
			</svg>
			...
			<svg
				className={`${styles.blobBrown} ${styles.blob}`}
				width="224"
				height="166"
				viewBox="0 0 224 166"
				fill="none"
			...
			<div className={styles.content}>
				<ul className={styles.www}>
					<li className={styles.item}>
						...
						<ul className={styles.artists}>
							{exhibition.artist_name.map((artist, i) => (
								<li key={`${exhibition.title}-artists-${i}`}>
									{artist}
									{exhibition.artist_name.length - 1 != i ? "," : ""}
								</li>
							))}
						</ul>
					</li>
				...
				{exhibition.image && (
					<Image
						...
						caption={exhibition.image_caption}
					/>
				)}
			...
			<div className={styles.cta}>
				<div>{mapLink ? <Link href={mapLink}>How to get there</Link> : null}</div>
			</div>
		</BaseModal>
	);
}
